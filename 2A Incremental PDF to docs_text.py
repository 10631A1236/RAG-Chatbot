...
# Directory path
directory_path = "/Volumes/development/dev_mpo_soil_curated/dev_mpo_soil_volume"
...
pdf_volume_path = "/Volumes/development/dev_mpo_soil_curated/dev_mpo_soil_volume"  # Specify the path to the PDF volume directory

# Get the list of already processed PDF files from the Delta table
processed_files = spark.sql(f"SELECT DISTINCT file_name FROM development.dev_mpo_soil_curated.docs_track").collect()
...
# MAGIC %sql
# MAGIC insert into development.dev_mpo_soil_curated.docs_text (text)
...
spark.sql("""
    INSERT INTO development.dev_mpo_soil_curated.docs_track
    SELECT * FROM temp_table
    WHERE NOT EXISTS (
        SELECT 1 FROM development.dev_mpo_soil_curated.docs_track
        WHERE temp_table.file_name = development.dev_mpo_soil_curated.docs_track.file_name
    )
""")